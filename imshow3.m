...
[Img, mask2, Rmin,Rmax,name,color] = ParseInputs(varargin{:});
if strcmp(color,'jet')
    slice_outline = bwperim(mask2,4);
    Img(slice_outline) = 1.1 * max(Img(:));
end
figure('Name',name); 
...
a=colormap(color);
axes('position',[0,0.2,1,0.8]), imshow(Img(:,:,S), [Rmin Rmax],'colormap',a)
rr = alphamask(mask2(:,:,S), [1 0 1], 0.5);
...
function SliceSlider (hObj,event, Img)
    ...
    imshow(Img(:,:,S),[Rmin Rmax],'colormap',a);
    rr = alphamask(mask2(:,:,S), [1 0 1], trs);
    caxis([Rmin Rmax])
    if sno > 1
...
function [Img, mask2, Rmin,Rmax,name,color] = ParseInputs(varargin)
...
if nargin ~= 2
  ...
  paramStrings = {'Disprange','colormap','Name'};
